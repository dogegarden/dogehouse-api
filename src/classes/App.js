...
class App {
    ...
    async registerRoutes() {
        ...
        function deprecated(call) {
            Logger.error(call + ' is deprecated. Please update.')
        }

        app.connect(process.env.DOGEHOUSE_TOKEN, process.env.DOGEHOUSE_REFRESH_TOKEN).then(async () => {
            Logger.info('Socket connected to DogeHouse.')
        });
        ...
        this.app.get('/v1/popularRooms', async (req, res) => {
            try {
                return res.json({rooms: await app.rooms.top})
            } catch(err) {
        ...
        this.app.get('/v1/user/:id', async (req, res) => {
            return res.status(200).json((await app.users.get(req.params.id)).toJson());
        });
        ...
        this.app.get('/v1/shields', async (req, res) => { // DEP!!
            deprecated('/v1/shields')
            try {
                let rooms = await app.rooms.top
        ...
        this.app.get('/v1/shields/users', async (req, res) => {
            try {
                let rooms = await app.rooms.top
        ...
        this.app.get('/v1/shields/premid/:presence', async (req, res) => {
            try {
                let data = await axios.get('https://api.premid.app/v2/presences/' + req.params.presence)
                return res.send({
                    schemaVersion: 1,
                    label: "PreMiD",
                    message: data.data.name + ' Presence',
                    logoSvg: premidsvg,
                    color: "purple"
                })
            } catch (err) {
    ...
    async listen(fn) {
        await Calls.formatBots() // Format bots on start.
